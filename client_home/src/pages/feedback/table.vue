<template>
  <view class="page_feedback diy_table diy_table_tml" id="feedback_table">
    <!-- 筛选模块(开始) -->
    <view>
            ...
            <view class="">
              ...
              <uni-forms :modelValue="query">
                    <uni-forms-item label="会员姓名" name="member_name">
                      ...
                      <uni-forms-item label="反馈类型" name="feedback_type">
                 <uni-data-select placeholder="请选择"
                        v-model="query['feedback_type']"
        ...
        <view>
          <view>
            <view class="table_padding_top">
              <navigator class="nav_left" url="/pages/feedback/view?" v-show="user_group == '管理员' || $check_action('/feedback/table','add') || $check_action('/feedback/view','add')">添加</navigator>
		...
		<view class="warp">
			<view class="container">
				<view v-for="(o,i) in list" class="row">
										<view v-if="1 && $check_field('get','member_account')" class="col">
						<view class="view">
                            ...
                            <span>会员账号</span>
								...
								<text>
									{{ get_user_member_account(o['member_account']) }}
													...
													<view v-if="0 && $check_field('get','member_name')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>会员姓名</span>
                          </view>
								...
								<text>
									{{ o["member_name"] }}
													...
													<view v-if="1 && $check_field('get','submission_time')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>提交时间</span>
                          </view>
								...
								<text>
									{{ $toTime(o["submission_time"],"yyyy-MM-dd hh:mm:ss") }}
													...
													<view v-if="1 && $check_field('get','feedback_type')" class="col">
						<view class="view">
                          <view class="diy_title" v-if="true">
                            <span>反馈类型</span>
                          </view>
                  ...
                  <view class="col">
                    <view class="view">
                      <view class="create_time_block diy_filed diy_time">
                        ...
                        <button class="delete_a_btn" @click="delInfo(i)" v-show="user_group == '管理员' || $check_action('/feedback/table','del') || $check_action('/feedback/view','del')">删除</button>
                      </view>
                    </view>
                  </view>
					<view class="uni-padding-wrap uni-common-mt">
					<navigator  :url="'/pages/feedback/view?' + field + '=' + o[field]"
                               v-show="user_group == '管理员' || $check_action('/feedback/table','set') || $check_action('/feedback/view','set') || $check_action('/feedback/view','get')" class="btn btn-primary">详情</navigator>
	        					</view>
				</view>
			</view>
		</view>
		...
		data() {
			return {
                ...
                list_user_member_account: [],
                                        // 反馈类型列表
                list_feedback_type: [{value:"",text:"全部"}],
                			}
		},
		methods: {
          search() {
