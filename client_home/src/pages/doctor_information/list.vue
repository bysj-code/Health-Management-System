<template>
	<view class="page_doctor_information diy_list diy_list_page div_list_tml" id="doctor_information_list">
		<!-- 筛选模块(开始) -->
		<view>
			<view class="container">
				<view>
					<view>
						<view class="">
							<!-- 搜索栏 -->
														<uni-search-bar placeholder="搜索医生编号" @confirm="search" @cancel="cancel" cancelText="取消"
											@input="input($event, 'doctor_no')">
								<uni-icons slot="searchIcon" color="#999999" size="18" type="home" />
							</uni-search-bar>
																			<uni-search-bar placeholder="搜索医生姓名" @confirm="search" @cancel="cancel" cancelText="取消"
											@input="input($event, 'doctors_name')">
								<uni-icons slot="searchIcon" color="#999999" size="18" type="home" />
							</uni-search-bar>
																				<uni-search-bar placeholder="搜索医生性别" @confirm="search" @cancel="cancel" cancelText="取消"
											@input="input($event, 'doctors_gender')">
								<uni-icons slot="searchIcon" color="#999999" size="18" type="home" />
							</uni-search-bar>
															<!-- /搜索栏 -->
						</view>
			...
			<view class="container">
				<view v-for="(o,i) in list" class="row box_style">
					...
					<view class="uni-padding-wrap uni-common-mt">
						<navigator :url="'./details?doctor_information_id=' + o['doctor_information_id']" class="btn btn-primary">查看详情</navigator>
...
<script>
	 ...
	 */
	get_praise(list){
		this.$get("~/api/praise/count_group?groupby=source_id&source_table=doctor_information", {}, (res) => {
			if(res.result){
				res.result.map((o)=>{
					for(var i = 0;i < list.length; i++){
						var oj = list[i];
						if(oj["doctor_information_id"] === o["source_id"]){
