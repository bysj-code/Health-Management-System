<template>
  <view id="home" class="home--new">
    ...
    <Card
      class="list_diy"
      title="医生信息推荐"
      url="/pages/doctor_information/list"
      :list="list_article"
      v-if="$check_action('/doctor_information/list', 'get')"
    >
	  <list_doctor_information :list="list_doctor_information"></list_doctor_information>
...
<script>
					...
					import list_doctor_information from "@/components/diy/list_doctor_information.vue";
      ...
      this.isAdmin = !this.isAdmin;
		...
		*/
		get_doctor_information() {
			let url = "~/api/doctor_information/get_list?";
															url = "~/api/doctor_information/get_list?orderby=hits desc";
										
			this.$get(url, {
											}, (json) => {
					if (json.result && json.result.list) {
					console.log("医生信息" ,json.result.list);
					this.list_doctor_information = json.result.list;
											this.list_doctor_information.map((o)=>{
							o["praise_len"];
						});
						this.get_praise(this.list_doctor_information ,"doctor_information" ,"41794");
									}
			});
		},
