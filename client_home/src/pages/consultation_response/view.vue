          ...
          <view class="">
            <uni-forms :modelValue="form"  v-if="is_view()">
              ...
              <uni-forms-item v-if="$check_field('get','member_account') || ($check_field('add','member_account') || $check_field('set','member_account'))" label="会员账号" name="member_account">
                  ...
                  v-if="user_group === '管理员' || (form['consultation_response_id'] && $check_field('set','member_account')) || (!form['consultation_response_id'] && $check_field('add','member_account'))"
                ></uni-data-select>
                <uni-data-select
                  v-model="form['member_account']"
                  :localdata="list_user_member_account"
              ...
              <uni-forms-item v-if="$check_field('get','member_name') || ($check_field('add','member_name') || $check_field('set','member_name'))" label="会员姓名" name="member_name">
                            <uni-easyinput type="text" v-model="form['member_name']" v-if="user_group === '管理员' || (form['consultation_response_id'] && $check_field('set','member_name')) || (!form['consultation_response_id'] && $check_field('add','member_name'))" :disabled="disabledObj['member_name_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','member_gender') || ($check_field('add','member_gender') || $check_field('set','member_gender'))" label="会员性别" name="member_gender">
                            <uni-easyinput type="text" v-model="form['member_gender']" v-if="user_group === '管理员' || (form['consultation_response_id'] && $check_field('set','member_gender')) || (!form['consultation_response_id'] && $check_field('add','member_gender'))" :disabled="disabledObj['member_gender_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','doctor_account') || ($check_field('add','doctor_account') || $check_field('set','doctor_account'))" label="医生账号" name="doctor_account">
                  ...
                  v-if="user_group === '管理员' || (form['consultation_response_id'] && $check_field('set','doctor_account')) || (!form['consultation_response_id'] && $check_field('add','doctor_account'))"
                ></uni-data-select>
                <uni-data-select
                  v-model="form['doctor_account']"
                  :localdata="list_user_doctor_account"
              ...
              <uni-forms-item v-if="$check_field('get','doctor_no') || ($check_field('add','doctor_no') || $check_field('set','doctor_no'))" label="医生编号" name="doctor_no">
                            <uni-easyinput type="text" v-model="form['doctor_no']" v-if="user_group === '管理员' || (form['consultation_response_id'] && $check_field('set','doctor_no')) || (!form['consultation_response_id'] && $check_field('add','doctor_no'))" :disabled="disabledObj['doctor_no_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','doctors_name') || ($check_field('add','doctors_name') || $check_field('set','doctors_name'))" label="医生姓名" name="doctors_name">
                            <uni-easyinput type="text" v-model="form['doctors_name']" v-if="user_group === '管理员' || (form['consultation_response_id'] && $check_field('set','doctors_name')) || (!form['consultation_response_id'] && $check_field('add','doctors_name'))" :disabled="disabledObj['doctors_name_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','consultation_topic') || ($check_field('add','consultation_topic') || $check_field('set','consultation_topic'))" label="咨询主题" name="consultation_topic">
                            <uni-easyinput type="text" v-model="form['consultation_topic']" v-if="user_group === '管理员' || (form['consultation_response_id'] && $check_field('set','consultation_topic')) || (!form['consultation_response_id'] && $check_field('add','consultation_topic'))" :disabled="disabledObj['consultation_topic_isDisabled']" />
                <!-- 仅查看 -->
              ...
              <uni-forms-item v-if="$check_field('get','content') || ($check_field('add','content') || $check_field('set','content'))" label="内容" name="content">
                    <uni-easyinput type="textarea" v-model="form['content']" v-if="user_group === '管理员' || (form['consultation_response_id'] && $check_field('set','content')) || (!form['consultation_response_id'] && $check_field('add','content'))" :disabled="disabledObj['content_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','content')">
                  {{ form['content'] }}
                </text>
                  </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','reply_time') || ($check_field('add','reply_time') || $check_field('set','reply_time'))" label="回复时间" name="reply_time">
                    <uni-datetime-picker v-if="user_group === '管理员' || (form['consultation_response_id'] && $check_field('set','reply_time')) || (!form['consultation_response_id'] && $check_field('add','reply_time'))" @change="changeLog($event,'reply_time')" v-model="form['reply_time']" type="datetime" :disabled="disabledObj['reply_time_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','reply_time')">
                  {{ form['reply_time'] }}
                </text>
                  </uni-forms-item>
              <uni-forms-item v-if="$check_field('get','reply') || ($check_field('add','reply') || $check_field('set','reply'))" label="回复" name="reply">
                    <uni-easyinput type="textarea" v-model="form['reply']" v-if="user_group === '管理员' || (form['consultation_response_id'] && $check_field('set','reply')) || (!form['consultation_response_id'] && $check_field('add','reply'))" :disabled="disabledObj['reply_isDisabled']" />
                <!-- 仅查看 -->
                <text v-else-if="$check_field('get','reply')">
                  {{ form['reply'] }}
                </text>
                  </uni-forms-item>
    ...
    return {
      ...
      form: {
            "member_account": 0, // 会员账号
                    ...
                    "content":  '', // 内容
                    "reply_time": this.$toTime(new Date().getTime(), "yyyy-MM-dd hh:mm:ss"),
                    "reply":  '', // 回复
      ...
      uni.chooseFile({
        ...
        success: function(res) {
          ...
          const uploadTask = uni.uploadFile({
            url: _self.$fullUrl('/api/consultation_response/upload?'),
    ...
    get_obj_before(param){
      ...
      for (var i=0;i<arr.length;i++){
        this.disabledObj[arr[i] + '_isDisabled'] = true
                                                                          ...
                                                                          if (this.form["reply_time"] && JSON.stringify(this.form["reply_time"]).indexOf("-")===-1) {
