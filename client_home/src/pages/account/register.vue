						...
						<view class="register_view">
							<uni-forms class="form_register" :rules="rules" :value="form" ref="form"
								...
								<uni-forms-item class="user_group" name="user_group" label="身份">
									...
									<!-- 会员用户 -->
								<form_member_users v-if="form.user_group === '会员用户'" @change="handleSubFormChange" :form="form_sub" ></form_member_users>
										<!-- 医生用户 -->
								<form_doctor_user v-if="form.user_group === '医生用户'" @change="handleSubFormChange" :form="form_sub" ></form_doctor_user>
...
<script>
	import mixin from "@/libs/mixins/page.js"
		/* 会员用户 */
	import form_member_users from "@/components/diy/form_member_users";
			/* 医生用户 */
	import form_doctor_user from "@/components/diy/form_doctor_user";
			...
			async submit_check(param){
				...
				if(!ret && user_group){
						...
						var form_sub = Object.assign({} ,this.form_sub);
							...
							if(user_group === "医生用户"){
						var form_sub = Object.assign({} ,this.form_sub);
											if(!ret && !form_sub["doctor_no"]){
							ret = "医生编号不能为空!";
						}
						else if(!ret){
							res = await this.$get("~/api/doctor_user/count?", {"doctor_no": form_sub["doctor_no"]});
							if(res.result){
								ret = "医生编号已存在!";
							}
						}
