<template>
	<view class="diy_details diy_div_consultation_information">
		<view v-if="Object.keys(obj).length!==0" class="warp">
			<view class="container">
				<view class="row">
					...
					<view v-if="$check_field('get','submission_time')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>提交时间:</span>
						</view>
							<view class="diy_field diy_datetime">
							<text>
								{{ $toTime(obj["submission_time"],"yyyy-MM-dd hh:mm:ss") }}
							</text>
						</view>
						</view>
			...
			<view class="container">
				<view class="row">
					<view class="col">
						<button type="button" class="diy_btn" @click="to_form('/pages/consultation_response/edit')" v-if="$check_action('/consultation_response/edit','add') || $check_action('/consultation_response/edit','set')" >医生回复</button>
					</view>
				</view>
			</view>
		</view>
...
<script>
	...
	export default {
		...
		methods: {
			...
			async get_obj_by_id(){
				var res = await this.$get("~/api/consultation_information/get_obj", {
					consultation_information_id:this.query.consultation_information_id
				})
		...
		async onLoad(){
			...
			} else if (this.query["consultation_information_id"] !==0) {
...
<style scoped>
	.div_consultation_information_details {}
