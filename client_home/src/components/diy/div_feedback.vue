<template>
	<view class="diy_details diy_div_feedback">
		<view v-if="Object.keys(obj).length!==0" class="warp">
			<view class="container">
				<view class="row">
					...
					<view v-if="$check_field('get','feedback_type')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>反馈类型:</span>
						</view>
							<view class="diy_field diy_text">
...
<script>
	...
	export default {
		...
		methods: {
			...
			async get_obj_by_id(){
				var res = await this.$get("~/api/feedback/get_obj", {
		...
		async onLoad(){
			...
			} else if (this.query["feedback_id"] !==0) {
