<template>
	<view class="diy_details diy_div_health_assessment">
		<view v-if="Object.keys(obj).length!==0" class="warp">
			<view class="container">
				<view class="row">
					...
					<view v-if="$check_field('get','evaluation_date')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>评估日期:</span>
						</view>
							<view class="diy_field diy_date">
							<text>
								{{ $toTime(obj["evaluation_date"],"yyyy-MM-dd") }}
							</text>
						</view>
						</view>
					<view v-if="$check_field('get','physical_assessment')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>体质评估:</span>
						</view>
							<view class="diy_field diy_desc">
							<text>
								{{ obj["physical_assessment"] }}
							</text>
						</view>
						</view>
					<view v-if="$check_field('get','psychological_assessment')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>心理评估:</span>
						</view>
							<view class="diy_field diy_desc">
							<text>
								{{ obj["psychological_assessment"] }}
							</text>
						</view>
						</view>
					<view v-if="$check_field('get','doctors_advice')" class="col-12 col-md-6">
						<view class="diy_title">
							<span>医生建议:</span>
						</view>
							<view class="diy_field diy_desc">
...
<script>
	...
	export default {
		...
		methods: {
			...
			async get_obj_by_id(){
				var res = await this.$get("~/api/health_assessment/get_obj", {
					health_assessment_id:this.query.health_assessment_id
				})
		...
		async onLoad(){
			...
			} else if (this.query["health_assessment_id"] !==0) {
