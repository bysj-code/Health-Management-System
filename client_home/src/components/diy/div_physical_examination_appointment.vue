<template>
	<view class="diy_details diy_div_physical_examination_appointment">
...
<script>
	...
	export default {
		...
		methods: {
			...
			async get_obj_by_id(){
				var res = await this.$get("~/api/physical_examination_appointment/get_obj", {
					physical_examination_appointment_id:this.query.physical_examination_appointment_id
				})
		...
		async onLoad(){
			...
			} else if (this.query["physical_examination_appointment_id"] !==0) {
...
<style scoped>
	.div_physical_examination_appointment_details {}
