	...
	<el-menu class="side el-menu-vertical-demo" default-active="/" @open="handleOpen" @close="handleClose"
		...
		<el-submenu index="user"
			...
			<el-menu-item index="/user/table" v-show="user_group == '管理员'"><span>管理员</span></el-menu-item>
						<el-menu-item index="/member_users/table" v-show="user_group == '管理员' || $check_action('/member_users/table')"><span>会员用户</span></el-menu-item>
							<el-menu-item index="/doctor_user/table" v-show="user_group == '管理员' || $check_action('/doctor_user/table')"><span>医生用户</span></el-menu-item>
				...
				for (var i = 0; i < tables.length; i++) {
					...
					if (o.path.indexOf("/table") !== -1) {
						let list = ["exam","article", "article_type", ,"comment","notice"];
							list.push("member_users");
								list.push("doctor_user");
