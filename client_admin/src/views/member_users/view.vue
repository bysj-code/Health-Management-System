		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				...
				<el-form-item label="会员姓名" prop="member_name">
												<el-input id="member_name" v-model="form_sub['member_name']" placeholder="请输入会员姓名"
							  v-if="user_group === '管理员' || (form_sub['member_users_id'] && $check_field('set','member_name')) || (!form_sub['member_users_id'] && $check_field('add','member_name'))"></el-input>
					<div v-else-if="$check_field('get','member_name')">{{form_sub['member_name']}}</div>
											</el-form-item>
			</el-col>
				...
				<el-form-item label="会员性别" prop="member_gender">
								<el-select id="member_gender" v-model="form_sub['member_gender']"
						v-if="user_group === '管理员' || (form_sub['member_users_id'] && $check_field('set','member_gender')) || (!form_sub['member_users_id'] && $check_field('add','member_gender'))">
						<el-option v-for="o in list_member_gender" :key="o" :label="o" :value="o">
						</el-option>
					</el-select>
					<div v-else-if="$check_field('get','member_gender')">{{form_sub['member_gender']}}</div>
							</el-form-item>
			</el-col>
...
<script>
	...
	export default {
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["member_users_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?member_users_id=" + form_sub["member_users_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/member_users/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/member_users/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/member_users/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/member_users/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/member_users/view','get');
