		...
		<el-form ref="form" :model="form" status-icon label-width="120px" v-if="is_view()">
			...
			<el-col :xs="24" :sm="12" :lg="8" class="el_form_item_warp">
				...
				<el-form-item label="医生编号" prop="doctor_no">
												<el-input id="doctor_no" v-model="form_sub['doctor_no']" placeholder="请输入医生编号"
							  v-if="user_group === '管理员' || (form_sub['doctor_user_id'] && $check_field('set','doctor_no')) || (!form_sub['doctor_user_id'] && $check_field('add','doctor_no'))"></el-input>
					<div v-else-if="$check_field('get','doctor_no')">{{form_sub['doctor_no']}}</div>
											</el-form-item>
			</el-col>
				...
				<el-form-item label="医生姓名" prop="doctors_name">
												<el-input id="doctors_name" v-model="form_sub['doctors_name']" placeholder="请输入医生姓名"
							  v-if="user_group === '管理员' || (form_sub['doctor_user_id'] && $check_field('set','doctors_name')) || (!form_sub['doctor_user_id'] && $check_field('add','doctors_name'))"></el-input>
					<div v-else-if="$check_field('get','doctors_name')">{{form_sub['doctors_name']}}</div>
											</el-form-item>
			</el-col>
				...
				<el-form-item label="医生性别" prop="doctors_gender">
								<el-select id="doctors_gender" v-model="form_sub['doctors_gender']"
						v-if="user_group === '管理员' || (form_sub['doctor_user_id'] && $check_field('set','doctors_gender')) || (!form_sub['doctor_user_id'] && $check_field('add','doctors_gender'))">
						<el-option v-for="o in list_doctors_gender" :key="o" :label="o" :value="o">
						</el-option>
					</el-select>
					<div v-else-if="$check_field('get','doctors_gender')">{{form_sub['doctors_gender']}}</div>
							</el-form-item>
			</el-col>
...
<script>
	...
	export default {
        ...
        form_sub = this.events("submit_before", Object.assign({}, form_sub)) || form_sub;
				if(form_sub["doctor_user_id"]){
					// 提交事件
					this.$post("~/api/" + this.group_table + "/set?doctor_user_id=" + form_sub["doctor_user_id"],form_sub,(res)=>{
			...
			is_view(){
				...
				if(!bl){
					bl = this.$check_action('/doctor_user/table','add');
					console.log(bl ? "你有表格添加权限视作有添加权限" : "你没有表格添加权限");
				}
				if(!bl){
					bl = this.$check_action('/doctor_user/table','set');
					console.log(bl ? "你有表格添加权限视作有修改权限" : "你没有表格修改权限");
				}
				if(!bl){
					bl = this.$check_action('/doctor_user/view','add');
					console.log(bl ? "你有视图添加权限视作有添加权限" : "你没有视图添加权限");
				}
				if(!bl){
					bl = this.$check_action('/doctor_user/view','set');
					console.log(bl ? "你有视图修改权限视作有修改权限" : "你没有视图修改权限");
				}
				if(!bl){
					bl = this.$check_action('/doctor_user/view','get');
